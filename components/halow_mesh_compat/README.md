...
for the MM6108** (see `../../docs/morse-support-inquiry.md` and
`../../docs/mesh-driver-port-plan.md`). Without that firmware, no
amount of host-driver code makes the chip's mesh RX path open.

When the firmware prerequisite is met, the port proceeds per
`../../docs/mesh-driver-port-plan.md`:

1. Vendor `dot11ah/` from `/tmp/morse_driver` (or equivalent upstream
...
fullmac MMFW chip firmware (Phase 4f-step1 through Phase 4f-step33), we
