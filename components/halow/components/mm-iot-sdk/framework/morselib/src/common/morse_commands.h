...
enum morse_cmd_id
{
    ...
    /* warthog mesh-support fork: 802.11s mesh configuration. Command ID and
     * struct layout taken from the GPL MorseMicro/morse_driver morse_commands.h
     * (autogenerated, semver 56.17.0). The MM6108 firmware (>=1.17.6) accepts
     * a mesh VIF — see docs/history/mesh-port-scope.md. */
    MORSE_CMD_ID_MESH_CONFIG = 0x0039,

    /* warthog mesh-support fork (step15) — RX-filter-related opcodes the
     * GPL Linux driver issues but morselib's embedded port doesn't expose.
     * Hypothesis: without explicit BSSID_SET (and possibly MCAST_FILTER),
     * the chip's RX filter rejects all foreign mesh BSS frames. The Linux
     * driver calls morse_cmd_set_bssid() in response to mac80211's
     * BSS_CHANGED_BSSID event, including for mesh interfaces. We've never
     * called it. */
    MORSE_CMD_ID_MCAST_FILTER = 0x003C,
    /* warthog mesh-support fork (step17) — BSS_BEACON_CONFIG. Linux's
     * morse_mac_ops_bss_info_changed fires this on BSS_CHANGED_BEACON_ENABLED
     * for ALL non-IBSS interfaces including mesh (mac.c:4062). It's separate
     * from MESH_CONFIG. The chip's beacon RX/TX path appears to be disabled
     * by default until this command is issued with enable=1. */
    MORSE_CMD_ID_BSS_BEACON_CONFIG = 0x003D,
    MORSE_CMD_ID_BSSID_SET = 0x0052,
};

/* warthog mesh-support fork: opcode for morse_cmd_req_mesh_config. */
...
/* warthog mesh-support fork: MESH_CONFIG request. Field order/sizes match the
 * GPL morse_driver's morse_cmd_req_mesh_config. Multi-byte fields are little-
 * endian on the wire; the caller applies htole16() (see mmdrv_mesh_config). */
struct MM_PACKED morse_cmd_req_mesh_config
{
    struct morse_cmd_header hdr;
    /** enum morse_cmd_mesh_config_opcode: START begins beaconing + IRQs. */
    uint8_t mesh_cfg_opcode;
    /** Flag: firmware generates mesh beacons when set. */
    uint8_t enable_beaconing;
    /** Mesh Beacon Collision Avoidance config bitmap (0 = MBCA disabled). */
    uint8_t mbca_config;
    /** Minimum gap between our beacon and a neighbour's, milliseconds. */
    uint8_t min_beacon_gap_ms;
    /** MBSS start-scan duration, milliseconds (0 = firmware default). */
    uint16_t mbss_start_scan_duration_ms;
    /** TBTT adjustment timer interval, milliseconds (0 = firmware default). */
    uint16_t tbtt_adj_timer_interval_ms;
};


struct MM_PACKED morse_cmd_resp_mesh_config
{
    struct morse_cmd_header hdr;
    uint32_t status;
};
...
/* warthog mesh-support fork (step15) — BSSID_SET (opcode 0x0052) request.
 * Linux driver layout per command.c::morse_cmd_set_bssid. Field is a 6-byte
 * MAC, no flags. The chip uses this as the RX filter BSSID. Setting to
 * broadcast (ff:ff:ff:ff:ff:ff) MAY put the chip into "accept-all-BSS"
 * mode — the experiment we're enabling. Setting to own_addr matches what
 * Linux does for mesh by default (BSS_CHANGED_BSSID with vif->bss_conf.bssid). */
...
/* warthog mesh-support fork (step17) — BSS_BEACON_CONFIG request (0x003D).
 * Single byte: enable. Linux driver layout per command.c::morse_cmd_config_
 * beacon_timer + morse_commands.h::struct morse_cmd_req_bss_beacon_config. */
