 ...
 * .send_mlme to push action frames (PLINK Open/Confirm/Close), .get_mac_addr
...
static void *mmwpas_init_mesh(void *ctx, const char *ifname)
{
    ...
    struct umac_data *umacd = umac_data_get_umacd();
    struct umac_supp_shim_data *data = umac_data_get_supp_shim(umacd);
    data->mesh_driver_ctx = ctx;
    return umacd;
}

static void mmwpas_deinit_mesh(void *priv)
{
    struct umac_data *umacd = (struct umac_data *)priv;
    struct umac_supp_shim_data *data = umac_data_get_supp_shim(umacd);
...
const struct wpa_driver_ops mmwlan_wpas_ops_mesh = {
     ...
     * set up any AP STA context. The chip crashed deep inside add_iface
     * with no useful state for follow-up. Lesson: only reuse AP ops where
