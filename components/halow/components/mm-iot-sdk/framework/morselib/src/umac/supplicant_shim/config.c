 ...
 * channel list (set via mmwlan_set_channel_list before mesh enable), not from
 * this config block — the supplicant doesn't drive HaLow channel selection
 * in our embedded flow. */
static bool wpa_config_read_mesh(struct wpa_config *config, bool ro)
{
    const struct mmwlan_mesh_args *args = umac_mesh_get_args();
    if (args == NULL)
    {
        MMLOG_ERR("mesh: wpa_config_read_mesh called before umac_mesh_enable_mesh\n");
        return false;
    }
    ...
    if (ssid == NULL)
    {
        MMLOG_ERR("Failed to add mesh network to supplicant\n");
        return false;
    }
    ...
    /* Hostap's mesh code reads ssid->ssid (the mesh ID) — the 802.11s spec
     * carries Mesh ID in the same wire format an infrastructure SSID uses,
     * so wpa_ssid reuses the same buffer. */
    ssid->ssid = (u8 *)os_malloc(args->mesh_id_len);
    if (ssid->ssid == NULL)
    {
        MMLOG_ERR("Failed to allocate mesh_id buffer for config\n");
        return false;
    }
    memcpy(ssid->ssid, args->mesh_id, args->mesh_id_len);
    ssid->ssid_len = args->mesh_id_len;

    /* Pull the country code from the chip's channel list so hostap's
     * regulatory checks line up. Same pattern as wpa_config_read_ap. */
    struct umac_data *umacd = umac_data_get_umacd();
    const struct mmwlan_s1g_channel_list *chan_list = umac_config_get_channel_list(umacd);
    if (chan_list != NULL)
    {
    ...
    /* Security: OPEN means no PMF / no SAE; SAE seeds the passphrase. */
    bool ok = config_populate_network_security(ssid,
                                               args->security_type,
                                               (const char *)args->passphrase,
                                               args->passphrase_len);
    if (!ok)
    {
        return false;
    ...
    if (args->security_type == MMWLAN_SAE)
    {
        config->pmf = MGMT_FRAME_PROTECTION_REQUIRED;
        config->sae_pwe = SAE_PWE_HASH_TO_ELEMENT;
    }

    MMLOG_INF("mesh config: mesh_id_len=%u security=%s country=%c%c\n",
              args->mesh_id_len,
...
const struct mmwlan_supp_config_entry mmwlan_wpa_config_mesh = {
    UMAC_SUPP_MESH_CONFIG_NAME,
    wpa_config_read_mesh,
};
