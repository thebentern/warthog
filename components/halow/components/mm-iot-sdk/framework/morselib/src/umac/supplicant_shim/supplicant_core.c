...
void umac_supp_process_mgmt_frame(struct umac_data *umacd, struct mmpktview *rxbufview)
{
     ...
     * mgmt frames here at all with two boards beaconing, the chip isn't
     * routing them to the supplicant — different problem than we thought. */
    const uint8_t *fc = (const uint8_t *)wpa_event_data.rx_mgmt.frame;
    MMLOG_INF("mgmt rx: fc=0x%02x%02x len=%zu (ap=%d sta=%d mesh=%d)\n",
              fc[0], fc[1], (size_t)wpa_event_data.rx_mgmt.frame_len,
    ...
    if (data->mesh_driver_ctx != NULL)
    {
        /* Hostap's event handler examines wpa_s->ifmsh and dispatches mesh
