...
morse_error_t morse_trns_read_le32(struct driver_data *driverd, uint32_t address, uint32_t *data)
{
     ...
     * loop, only one boot-time read is a status check at 0x1005807c.
     * No reads carry mode-configuration data. */
...
morse_error_t morse_trns_write_le32(struct driver_data *driverd, uint32_t address, uint32_t data)
{
     ...
     *
     *   WR#2  0x10058094 = 0x00000000  IRQ enable mask (top)
     *   WR#3  0x10058098 = 0x00000000  IRQ enable mask (bottom)
     ...
     *   WR#7  0x1005406c = 0x000000ef  config register (feature enable)
     *   WR#8  0x02000000 = 0x00000001  CHIP RESET RELEASE / BOOT TRIGGER
     *   (then 22 reads polling 0x10054d40 for chip-ready)
     *   WR#9..#18  paired writes to 0x100a6058/0x100a605c (queue/PG setup)
     *   WR#19, #22  0x100ac110 (pager: 0x80107f50, 0x801085c0)
     *   WR#20, #23  0x100a6058 = 0x20000001
     *   WR#21, #24  0x100ac310 (pager: 0x88000000, 0x80000004)
     *
     ...
     *   - 0x1005406c = 0xef config write — looks like SDK-specific
     *
     * NO writes contain values that look like a "fullmac-strict-RX-mode"
     * bit. The 0x000000ef feature register at 0x1005406c is the most
     * suspicious — it's only 8 bits, 7 of 8 low bits set (binary
     * 11101111). One bit is clear (bit 4). If bit 4 == "permissive RX
     * mode" and "clear" == "permissive disabled" that COULD be the lock,
     * but we have no documentation to confirm.
     *
     * Without on-Linux capture for byte-by-byte comparison, we cannot
     * definitively identify a "this register write is the asymmetry"
     * — but the SDK's boot init writes look like generic chip-init
     * operations, not mode-switch ones. */
