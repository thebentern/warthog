...
void morse_skbq_process_rx(struct driver_data *driverd, struct mmpkt *mmpkt)
{
     ...
     * MGMT (0x4) or BEACON (0x3), the chip IS delivering frames and the
     * drop is host-side. Logs the first time each distinct channel is seen,
     * plus a periodic histogram. Cheap (first-sight is ~6 lines total). */
    {
        static uint8_t s_seen_channels[256] = {0};
        static uint32_t s_chan_hist[256] = {0};
        ...
        if (!s_seen_channels[channel])
        {
            ...
            MMLOG_ERR("RXCHAN first-sight chan=0x%02x (page#%lu)\n",
                      (unsigned)channel, (unsigned long)s_rx_pages);
        }
        if ((s_rx_pages % 256) == 0)
        {
            MMLOG_ERR("RXCHAN hist@%lu: cmd(fe)=%lu txstat(ff)=%lu data(00)=%lu "
                      "beacon(03)=%lu mgmt(04)=%lu\n",
                      (unsigned long)s_rx_pages,
                      (unsigned long)s_chan_hist[0xfe],
                      (unsigned long)s_chan_hist[0xff],
                      (unsigned long)s_chan_hist[0x00],
                      (unsigned long)s_chan_hist[0x03],
                      (unsigned long)s_chan_hist[0x04]);
        }
    }
