...
void mmhal_uart_init(mmhal_uart_rx_cb_t rx_cb, void *rx_cb_arg)
{
    ...
    LL_LPUART_ClearFlag_ORE(UART_INTERFACE);
    LL_LPUART_EnableIT_RXNE(UART_INTERFACE);
}
...
void mmhal_uart_tx(const uint8_t *tx_data, size_t length)
{
    while (length-- != 0)
    {
        while (!LL_LPUART_IsActiveFlag_TXE(UART_INTERFACE))
        {
        }
        LL_LPUART_TransmitData8(UART_INTERFACE, *tx_data++);
    }
}
...
{
    while (LL_LPUART_IsEnabledIT_RXNE(UART_INTERFACE) &&
           LL_LPUART_IsActiveFlag_RXNE(UART_INTERFACE))
    {
