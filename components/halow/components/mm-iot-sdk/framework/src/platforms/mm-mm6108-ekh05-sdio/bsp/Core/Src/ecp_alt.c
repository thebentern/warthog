...
/*
 * Hardware crypto doesn't support compressed point format by default, so we need to grab
 * the software crypto functions to derive the Y values from mbedtls.
 */
                                  ...
                                  size_t ilen)
{
    ...
    if (mbedtls_ecp_get_type(grp) == MBEDTLS_ECP_TYPE_MONTGOMERY)
    {
        ...
        {
            /* Set most significant bit to 0 as prescribed in RFC7748 par. 5 */
 ...
 */
int mbedtls_ecp_export(const mbedtls_ecp_keypair *key,
                       mbedtls_ecp_group *grp,
                       mbedtls_mpi *d,
                       mbedtls_ecp_point *Q)
{
