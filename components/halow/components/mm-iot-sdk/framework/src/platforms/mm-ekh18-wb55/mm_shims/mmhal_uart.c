...
bool mmhal_uart_set_deep_sleep_mode(enum mmhal_uart_deep_sleep_mode mode)
{
    if (mode == MMHAL_UART_DEEP_SLEEP_ONE_SHOT)
    {
        ...
        LL_SYSCFG_SetEXTISource(LOG_USART_RX_EXTI_Port, LOG_USART_RX_EXTI_Line);
        ...
        LL_EXTI_ClearFlag_0_31(LOG_USART_RX_IRQ_LINE);
...
{
    if (LL_EXTI_IsActiveFlag_0_31(LOG_USART_RX_IRQ_LINE) != RESET)
    {
