...
#error \
    "MBEDTLS_ECP_NIST_OPTIM defined, but it cannot coexist with ST alternative ECP implementation"
#endif
                          ...
                          size_t nlen)
{
    ...
    grp->st_p = (uint8_t *)mbedtls_calloc(grp->st_modulus_size, sizeof(uint8_t));
    MBEDTLS_MPI_CHK((grp->st_p == NULL) ? MBEDTLS_ERR_ECP_ALLOC_FAILED : 0);
    mbedtls_mpi_write_binary(&grp->P, grp->st_p, grp->st_modulus_size);

    grp->st_a_abs = (uint8_t *)mbedtls_calloc(grp->st_modulus_size, sizeof(uint8_t));
    ...
    grp->st_b = (uint8_t *)mbedtls_calloc(grp->st_modulus_size, sizeof(uint8_t));
    MBEDTLS_MPI_CHK((grp->st_b == NULL) ? MBEDTLS_ERR_ECP_ALLOC_FAILED : 0);
    mbedtls_mpi_write_binary(&grp->B, grp->st_b, grp->st_modulus_size);

    grp->st_gx = (uint8_t *)mbedtls_calloc(grp->st_modulus_size, sizeof(uint8_t));
    ...
    grp->st_gy = (uint8_t *)mbedtls_calloc(grp->st_modulus_size, sizeof(uint8_t));
    ...
    grp->st_n = (uint8_t *)mbedtls_calloc(grp->st_order_size, sizeof(uint8_t));
...
static int ecp_use_curve25519(mbedtls_ecp_group *grp)
{
    ...
    MBEDTLS_MPI_CHK(mbedtls_mpi_read_string(&grp->N, 16, "14DEF9DEA2F79CD65812631A5CF5D3ED"));
