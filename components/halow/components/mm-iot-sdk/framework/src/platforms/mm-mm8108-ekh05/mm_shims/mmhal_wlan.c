...
/** Enable GPIO pins required for MM-Chip. */
static void wlan_hal_gpio_init(void)
{
    /* Configure MM-Chip GPIOs:
     * - Busy pin + IRQ
     * - SPI_IRQ pin + IRQ
     * - SPI CS pin
     * - Wake pin
     *
     * Note: Direct copy from main.c:MX_GPIO_Init() with unneeded config removed.
     */

    LL_EXTI_InitTypeDef EXTI_InitStruct = { 0 };
    LL_GPIO_InitTypeDef GPIO_InitStruct = { 0 };

    /* Note: GPIO clocks already enabled. */
    ...
    /* Note: EXTI Source already set (never unset). */

    /**/
    EXTI_InitStruct.Line_0_31 = SPI_IRQ_LINE;
    ...
    /**/
    EXTI_InitStruct.Line_0_31 = BUSY_IRQ_LINE;
    EXTI_InitStruct.LineCommand = ENABLE;
    EXTI_InitStruct.Mode = LL_EXTI_MODE_IT;
...
void wlan_gpio_deinit(void)
{
    ...
    LL_GPIO_ResetOutputPin(BUSY_GPIO_Port, BUSY_Pin);
    LL_GPIO_Init(BUSY_GPIO_Port, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = SPI_IRQ_Pin;
    LL_GPIO_ResetOutputPin(SPI_IRQ_GPIO_Port, SPI_IRQ_Pin);
    LL_GPIO_Init(SPI_IRQ_GPIO_Port, &GPIO_InitStruct);
...
/** Disable SPI peripheral + pins to prevent current leakage. */
static void wlan_hal_spi_deinit(void)
{
    LL_GPIO_InitTypeDef GPIO_InitStruct = { 0 };
    NVIC_DisableIRQ(SPI_PERIPH_IRQn);

    LL_SPI_DeInit(SPI_PERIPH);
    LL_DMA_DisableIT_TC(SPI_DMA_PERIPH, SPI_RX_DMA_CHANNEL);
    LL_DMA_DeInit(SPI_DMA_PERIPH, SPI_RX_DMA_CHANNEL);
    LL_DMA_DeInit(SPI_DMA_PERIPH, SPI_TX_DMA_CHANNEL);
...
{
    /* Initialise connections to MM-Chip before enabling chip. */
    wlan_hal_gpio_init();
    SPI_PERIPH_INIT();
...
void mmhal_wlan_deinit(void)
{
    ...
    /* Disable connections to MM-Chip once disabled to prevent leakage. */
    wlan_hal_spi_deinit();
    wlan_gpio_deinit();
}
