...
//printf("SDMMC init: CLKCR=%08x; Init.buswide=%08x\n", SDMMCx->CLKCR, Init.BusWide);
...
/**
  ...
  * @retval SD Card error state
  */
uint32_t SDMMC_GetCmdResp4(SDMMC_TypeDef *SDMMCx, uint32_t Timeout)
{
  uint32_t response_r4;
  uint32_t sta_reg;
...
//   if (SDMMC_GetCommandResponse(SDMMCx) != SDMMC_CMD_SDMMC_SEN_OP_COND)
//   {
//     return SDMMC_ERROR_CMD_CRC_FAIL;
//   }

  /* We have received response, retrieve it for analysis  */
  response_r4 = SDMMC_GetResponse(SDMMCx, SDMMC_RESP1);
  (void)response_r4;
  printf("    R4: %08lx\n", response_r4);
  if ((response_r4 & 0x80000000) == 0)
  {
    printf("    Not ready\n");
    return SDMMC_ERROR_CMD_CRC_FAIL; // TODO: for want of a better option for now
  }

  return SDMMC_ERROR_NONE;
}

uint32_t SDMMC_CmdSendOperCond(SDMMC_TypeDef *SDMMCx)
{
  SDMMC_CmdInitTypeDef  sdmmc_cmdinit;
  uint32_t errorstate;

  sdmmc_cmdinit.Argument         = 0xfc0000; // 3.0-3.6V just something I chose arbitrarily. No 1.8V sw request
  ...
  errorstate = SDMMC_GetCmdResp4(SDMMCx, SDMMC_CMDTIMEOUT);
...
static uint32_t SD_PowerON(SD_HandleTypeDef *hsd)
{
  ...
  /* CMD5 */
  printf("Send CMD5\n");
  errorstate = SDMMC_CmdSendOperCond(hsd->Instance);
  printf("CMD5 %lu\n", errorstate);
  if (errorstate != HAL_SD_ERROR_NONE)    {
      return errorstate;
  }

//   errorstate = SDMMC_GetCmdResp4(hsd->Instance, SDMMC_CMDTIMEOUT);
//   if (errorstate != HAL_SD_ERROR_NONE)    {
//       return errorstate;
//   }
