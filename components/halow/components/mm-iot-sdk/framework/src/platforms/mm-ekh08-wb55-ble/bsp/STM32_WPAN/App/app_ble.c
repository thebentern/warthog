...
static struct mmosal_mutex *MtxHciId = NULL;
static struct mmosal_sem *SemHciId = NULL;
...
static void hci_user_event_task(void *arg);
...
void APP_BLE_Init(void)
{
   ...
   */
  hci_task_handle = mmosal_task_create(hci_user_event_task, 0, MMOSAL_TASK_PRI_LOW,
                                       CFG_HCI_USER_EVT_PROCESS_STACK_SIZE,
   ...
   */
  adv_task_handle = mmosal_task_create(adv_update_task, 0, MMOSAL_TASK_PRI_LOW,
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          if (BleApplicationContext.Device_Connection_Status == APP_BLE_LP_CONNECTING)
          {
            ...
            APP_DBG_MSG("     - Connected as client\n");
            BleApplicationContext.Device_Connection_Status = APP_BLE_CONNECTED_CLIENT;
          }
          else
          {
            /* Connection as server */
            APP_DBG_MSG("     - Connected as server\n");
...
static void Ble_Tl_Init(void)
{
  ...
  MtxHciId = mmosal_mutex_create("MtxHciId");
  SemHciId = mmosal_sem_create(1, 0, "SemHciId"); /*< Create the semaphore and make it busy at initialization */
...
{
  mmosal_sem_give_from_isr(SemHciId);
...
{
  mmosal_sem_wait(SemHciId, UINT32_MAX);
