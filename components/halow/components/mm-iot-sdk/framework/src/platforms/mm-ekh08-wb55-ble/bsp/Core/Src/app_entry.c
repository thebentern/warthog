...
static void appe_Tl_Init(void)
{
  ...
  MtxShciId = mmosal_mutex_create("MtxShciId");
  MMOSAL_MUTEX_GET_INF(MtxShciId);
  SemShciId = mmosal_sem_create(1, 0, "SemHciId"); /*< Create the semaphore and make it busy at initialization */

  shci_task_handle = mmosal_task_create(shci_user_event_task, 0, MMOSAL_TASK_PRI_LOW,
...
static void APPE_SysStatusNot(SHCI_TL_CmdStatus_t status)
{
  switch (status)
    ...
    case SHCI_TL_CmdAvailable:
        MMOSAL_MUTEX_RELEASE(MtxShciId);
        break;
...
{
    mmosal_sem_give_from_isr(SemShciId);
    return;
}

void shci_cmd_resp_wait(uint32_t timeout)
{
    mmosal_sem_wait(SemShciId, UINT32_MAX);
    return;
}
