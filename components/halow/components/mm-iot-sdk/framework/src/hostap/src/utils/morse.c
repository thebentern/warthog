...
int morse_calculate_primary_s1g_channel_jp(int op_bw_mhz, int pr_bw_mhz,
		int s1g_op_chan, int pr_1mhz_chan_idx);
int morse_calculate_primary_s1g_channel(int op_bw_mhz, int pr_bw_mhz,
		int s1g_op_chan, int pr_1mhz_chan_idx);
...
int morse_ht_chan_to_ht_chan_center(struct hostapd_config *conf, int ht_chan)
{
	...
	if (conf->ieee80211ac) {
		...
		case CHANWIDTH_80MHZ:
			...
			ht_center_chan = ht_chan -
					vht_80_pri_1mhz_offset[conf->s1g_prim_1mhz_chan_loc];
		...
		case CHANWIDTH_160MHZ:
			if (morse_ht_chan_vht160(ht_chan))
				return ht_chan;
			ht_center_chan = ht_chan -
					vht_160_pri_1mhz_offset[conf->s1g_prim_1mhz_chan_loc];
	...
	if (conf->secondary_channel) {
		ht_center_chan = ht_chan - ht_40_pri_1mhz_offset[conf->s1g_prim_1mhz_chan_loc];
...
int morse_ht_center_chan_to_ht_chan(struct hostapd_config *conf, int ht_chan)
{
	...
	if (conf->secondary_channel) {
		if (morse_ht_chan_ht40(ht_chan))
			return ht_chan + ht_40_pri_1mhz_offset[conf->s1g_prim_1mhz_chan_loc];
	}
...
int morse_s1g_get_primary_channel(struct hostapd_config *conf, int bw)
{
	...
	if (conf->s1g_prim_1mhz_chan_loc >= op_bw)
		return MORSE_INVALID_CHANNEL;

	return morse_cc_get_primary_s1g_channel(op_bw,
						bw,
						s1g_op_chan,
						conf->s1g_prim_1mhz_chan_loc,
						conf->op_country);
}
				...
				struct hostapd_config *conf)
{
	...
	conf->s1g_prim_1mhz_chan_loc = s1g_prim_1mhz_chan_index;
	...
	*oper_chwidth = morse_s1g_op_class_to_ch_width(conf->s1g_op_class);
	...
	if (*oper_chwidth != 1)
		conf->secondary_channel = (conf->s1g_prim_1mhz_chan_loc % 2 ? -1 : 1);
...
int morse_s1g_validate_csa_params(struct hostapd_iface *iface,	struct csa_settings *settings)
{
	...
	settings->s1g_freq_params.s1g_prim_1mhz_channel_loc = s1g_prim_channel_index;
	/*
