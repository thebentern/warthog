...
"""
Emmet/Ace set 1MHz control response preamble enabled.
...
def _main():
    ...
    group.add_argument("--enable", "-e", action="store_true", dest="enabled",
                       help="Enable 1MHz control response preamble")
    group.add_argument("--disable", "-d", action="store_true", dest="enabled",
                       help="Disable 1MHz control response preamble")
    ...
    dutif.exec("wlan/set_control_response_preamble_1mhz_out_en", enabled=args.enabled)
    print(f"1MHz control response preamble {'enabled' if args.enabled else 'disabled'}")
