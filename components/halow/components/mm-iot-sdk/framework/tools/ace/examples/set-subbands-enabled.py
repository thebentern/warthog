...
#
"""
Emmet/Ace set sub-bands enabled example.
...
def _main():
    ...
    group.add_argument("--enable", "-e", action="store_true",
                       help="Enable sub-band support")
    group.add_argument("--disable", "-d", action="store_true",
                       help="Disable sub-band support")
    ...
    dutif.exec("wlan/set_subbands_enabled", subbands_enabled=args.enable)
    if args.enable:
        print("Sub-band support enabled")
    else:
        print("Sub-band support disabled")
