...
#
"""
Emmet/Ace add or remove a static ARP entry
"""
...
from acelib import DutIf, ip_addr_to_bytes, mac_addr_str_to_bytes  # noqa: E402
...
def _main():
    ...
    parser.add_argument("-i", "--ipaddr", required=True,
                        help="IPv4 address in dotted decimal notation (x.x.x.x)")
    parser.add_argument("-m", "--macaddr",
                        help="MAC address (xx:xx:xx:xx:xx:xx - case insensitive)")
    parser.add_argument("-r", "--remove", action="store_true",
                        help="Remove an existing static ARP entry")
    ...
    mac_addr = mac_addr_str_to_bytes(args.macaddr) if args.macaddr else b""
    ip_addr = ip_addr_to_bytes(args.ipaddr) if args.ipaddr else b"\0\0\0\0"

    dutif.exec("lwip/set_static_arp",
               remove=args.remove,
               ip_addr=ip_addr,
               mac_addr=mac_addr)
