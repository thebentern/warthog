...
def _main():
    ...
    rsp = dutif.exec("lwip/get_ip_config")
    rsp.dhcp_enabled = 0 if rsp.dhcp_enabled is None else 1

    print(f'DHCP {"enabled" if rsp.dhcp_enabled else "disabled"}')
    print(f"IP: {ip_addr_bytes_to_str(rsp.ip_addr)}, Netmask: {ip_addr_bytes_to_str(rsp.netmask)}, Gateway: {ip_addr_bytes_to_str(rsp.gateway_addr)}")  # noqa: E501
