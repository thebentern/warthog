...
#
"""
Emmet/Ace get duty cycle stats example
"""
...
def _main():
    ...
    rsp = dutif.exec("wlan/get_duty_cycle_stats")

    print(f"Duty cycle: {rsp.duty_cycle/100.0}%", end="")  # no new line, optional disabled tag

    if (rsp.duty_cycle == 10000):
        print(" (disabled)")
        return

    print("\nDuty cycle mode: ", end="")  # no new line, conditional mode append

    if (rsp.mode):
        print("Burst")
        air_time_us = rsp.burst_airtime_remaining_us or 0
        total_time_us = rsp.burst_window_duration_us
        print(f"Air time remaining: {air_time_us/1e6} sec")
        print(f"Duty cycle window: {total_time_us/1e6} sec")
    else:
        print("Spread")
