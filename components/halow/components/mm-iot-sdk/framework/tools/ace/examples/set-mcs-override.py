...
#
"""
Emmet/Ace set mcs override example
"""
...
def _main():
    ...
    parser.add_argument("mcs_rate",
                        help="selects mcs mode(0~9) or type (auto) for auto rate control")
    ...
    if args.mcs_rate == "auto":
        dutif.exec("wlan/set_mcs_override", mcs=-1)
        print("MCS set to auto")
    else:
        mcs = int(args.mcs_rate)
        dutif.exec("wlan/set_mcs_override", mcs=mcs)
        print(f"MCS set to {mcs}")
