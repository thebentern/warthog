...
"""
Emmet/Ace example to set STA autoconnect mode.
...
def _main():
    ...
    group.add_argument("--disable", "-d", action="store_true",
                       help="Do not automatically reconnect on connection loss")
    ...
    if args.enable:
        print("STA autoconnect mode set to enabled")
        mode = "STA_AUTOCONNECT_ENABLED"
    elif args.disable:
        print("STA autoconnect mode set to disabled")
        mode = "STA_AUTOCONNECT_DISABLED"

    dutif.exec("wlan/set_sta_autoconnect", sta_autoconnect_mode=mode)
