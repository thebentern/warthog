...
#
"""
Emmet/Ace set WNM Sleep enabled example.
...
def _main():
    ...
    group.add_argument("--enable", "-e", action="store_true",
                       help="Enable WNM Sleep")
    group.add_argument("--disable", "-d", action="store_true",
                       help="Disable WNM Sleep")
    parser.add_argument("--enable-chip-power-down", "-c", action="store_true",
                        help="Power down MM chip during WNM sleep.")
    ...
    if args.enable_chip_power_down:
        chip_power_down = True
    else:
        chip_power_down = False

    dutif.exec("wlan/set_wnm_sleep_enabled",
               wnm_sleep_enabled=args.enable, chip_power_down_enabled=chip_power_down)
    if args.enable:
        print("WNM sleep enabled")
    else:
        print("WNM sleep disabled")
