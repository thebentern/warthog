# https://github.com/platformio/platform-ststm32/blob/4f5783554afd2b4d50db3eba5266dede9a9e6bf4/platform.py
#
...
class Ststm32Platform(PlatformBase):
    ...
    def configure_default_packages(self, variables, targets):
        ...
        if "cmsis" in frameworks:
            assert build_mcu, (f"Missing MCU field for {board}")
        ...
        if "stm32cube" in frameworks:
            assert build_mcu, (f"Missing MCU field for {board}")
            device_package = f"framework-stm32cube{build_mcu[5:7]}"
        ...
        for link in ("blackmagic", "jlink", "stlink", "cmsis-dap"):
            ...
            if link == "blackmagic":
                ...
                assert debug.get("jlink_device"), (
                    f"Missed J-Link Device ID for {board.id}")
                debug["tools"][link] = {
                    "server": {
                        "package": "tool-jlink",
                        "arguments": [
        ...
        if debug_config.speed:
            ...
            if "openocd" in server_executable:
                debug_config.server["arguments"].extend(
                    ["-c", f"adapter speed {debug_config.speed}"]
                )
